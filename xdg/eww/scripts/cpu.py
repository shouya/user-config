...
def change_policy():
    ...
    sys_path = "/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor"
    with open(sys_path, "w") as f:
        f.write(new_governor)

    subprocess.check_call(
        ["notify-send", f"Switching cpu governor policy to {new_governor}"]
    )
    ...
    if new_governor == "performance":
        ...
        subprocess.check_call(
            [
                ...
                "sh",
                "-c",
                f"echo powersave > /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor",
            ]
        )
